...
/* Change package from package gnu.getopt to */
package ucar.jpic;
  ...
  if (ambig && !exact)
    {
      if (opterr)
        {
          System.err.printf("%s ''%s'' is ambiguous\n",progname,argv[optind]);
        }
              ...
              if (opterr)
                {
                  ...
                  if (argv[optind - 1].startsWith("--"))
                    {
		      System.err.printf("%s ''--%s'' does not allow an argument\n",progname,pfound.name);
                    }
                  // +option or -option
                  else
                    {
		      System.err.printf("%s ''%c%s'' does not allow an argument\n",progname,argv[optind-1].charAt(0),pfound.name);
                    }
                 }
      ...
      else if (pfound.has_arg == LongOpt.REQUIRED_ARGUMENT)
        {
          if (optind < argv.length)
          ...
          else
            {
              if (opterr)
                {
	          System.err.printf("%s ''%s'' requires an argument\n",progname,argv[optind-1]);
                }
          ...
          if (opterr)
            {
              if (argv[optind].startsWith("--"))
                {
	          System.err.printf("%s ''%s'' unrecognized option\n",progname,nextchar);
                }
              else
                {
	          System.err.printf("%s ''%c%s'' unrecognized option\n",progname,argv[optind].charAt(0),nextchar);
                }
            }
      ...
      if (opterr)
        {
	      ...
	      System.err.printf("%s illegal option -- ''%c''\n",progname,(char)c);
            }
          else
            {
	      System.err.printf("%s invalid option -- ''%c''\n",progname,(char)c);
            }
        }
      ...
      // No further cars in this argv element and no more argv elements
      else if (optind == argv.length)
        {
          if (opterr)
            {
              // 1003.2 specifies the format of this message. 
	      System.err.printf("%s option requires an argument -- ''%c''\n",progname,(char)c);
            }
        ...
        StringBuilder flag, int val) throws IllegalArgumentException
{
  ...
  if ((has_arg != NO_ARGUMENT) && (has_arg != REQUIRED_ARGUMENT) 
	...
	String.format("=Invalid value %d for parameter 'has_arg'\n",has_arg));
    }
